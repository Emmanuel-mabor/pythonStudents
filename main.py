...
# logging
logging.basicConfig(filename='student_registration.log', level=logging.INFO, format='%(asctime)s - %(name)s - %(levelname)s - %(message)s')
...
def edit_student():
    ...
    if selected_item:
        ...
        # Retrieve the student's information for editing
...
def log_out():
    # Close the current student registration script and open the home.py script
...
# window size to fit the screen
screen_width = root.winfo_screenwidth()
screen_height = root.winfo_screenheight()
root.geometry(f"{screen_width}x{screen_height}")

# font for labels, buttons, and entry widgets
...
student_id_entry = tk.Entry(root, width=60)
department_label = tk.Label(root, text="Department:")
department_entry = tk.Entry(root, width=60)
course_label = tk.Label(root, text="Course:")
course_entry = tk.Entry(root, width=60)
...
# Initialize the database connection and create the student table
conn = sqlite3.connect("student_database.db")
cursor = conn.cursor()
create_student_table(cursor)

# Show the initial student data
view_students()
root.mainloop()
conn.close()
